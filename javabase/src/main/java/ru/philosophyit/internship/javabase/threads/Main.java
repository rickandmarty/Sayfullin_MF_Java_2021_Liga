...
public class Main {
    ...
    // --- Данный метод позволяет выполняться переданным заданиям.
    // --- Новые задания не будут приняты.
    ...
    // --- В методе startSomeDaemon() бесконечный цикл, который создает потоки.


    // Почему при работе тасок executorService в консоль в секунду попадает всего 4 сообщения,
    // тогда как тасок в executorService - 16?
    // --- Потому что при создании executorService было добавлено только 4 потока.
